...
export default function DashboardFlavors({ flavors: flavorsProp, setFlavors: setFlavorsProp }) {
  const formId = useId();

  const pageSize = 8;
  const [page, setPage] = useState(1);

  const [localFlavors, setLocalFlavors] = useState(() => DEFAULT_FLAVORS.map((f) => ({ ...f })));
  const flavors = flavorsProp ?? localFlavors;
  const setFlavors = setFlavorsProp ?? setLocalFlavors;
  ...
  const openEdit = (id) => {
    const current = flavors.find((f) => f.id === id);
    if (!current) return;
    setModalMode("edit");
    setEditingId(id);
  ...
  const upsert = () => {
    ...
    if (!name) {
      window.alert("Please enter flavor name.");
      return;
    }

    const norm = normalizeFlavorName(name);
    const isDuplicate = flavors.some((f) => {
      if (modalMode === "edit" && f.id === editingId) return false;
      return normalizeFlavorName(f.name) === norm;
    });

    if (isDuplicate) {
      window.alert("This flavor already exists.");
      return;
    }

    if (modalMode === "add") {
      const id = `flav-${Date.now()}`;
      setFlavors((prev) => [{ id, name, isActive: true }, ...prev]);
      closeModal();
      return;
    }

    if (modalMode === "edit" && editingId) {
      setFlavors((prev) => prev.map((f) => (f.id === editingId ? { ...f, name } : f)));
      closeModal();
    }
  };
  ...
  const onDelete = (id) => {
    const current = flavors.find((f) => f.id === id);
    const ok = window.confirm(`Delete flavor “${current?.name ?? ""}”?`);
    if (!ok) return;
      ...
      <section className="bg-white rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-[#ececec] flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3">
            <h3 className="text-base font-bold text-[#3b2a23]">Flavor List</h3>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#fffaf9] text-[#3b2a23]">
              <tr>
                ...
                <th className="text-left font-bold px-6 py-4 whitespace-nowrap">Flavor Name</th>
          ...
          <div className="relative w-full max-w-md rounded-lg border border-[#ececec] bg-white shadow p-6">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h4 className="text-base font-bold text-[#3b2a23]">
                  {modalMode === "add" ? "Add Flavor" : "Edit Flavor"}
                </h4>
                <div className="text-xs text-[#7c6a5a] mt-1">Enter flavor name and save.</div>
              </div>
              <button
                type="button"
                onClick={closeModal}
            ...
            <div className="mt-5">
              <label htmlFor={`${formId}-name`} className="text-sm font-semibold text-[#3b2a23]">
                Flavor name <span className="text-[#ab8351]">*</span>
              </label>
              <div className="mt-2">
                <TextInput
                  id={`${formId}-name`}
